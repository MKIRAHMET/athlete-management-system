...
$new_category = $_POST['new_category']; // Assuming you have a field for new category
...
if ($category === 'other' && !empty($new_category)) {
    ...
    if ($stmt->execute()) {
        ...
        // Use the newly inserted category ID for the announcement
...
// Prepare and execute SQL statement to insert the announcement into the database
$sql = "INSERT INTO announcements (title, content, status, publication_date, category, tags, image_url) 
        VALUES (?, ?, ?, ?, ?, ?, ?)";
